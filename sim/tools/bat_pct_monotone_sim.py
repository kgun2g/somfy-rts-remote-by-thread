...
#     LO/HI(3913/3944)가 있는 3850~3980 구간은 어느 값에서도 안 바뀌므로
#     이 시뮬레이터의 6.5mV/%p 전제는 영향받지 않는다.
...
class Gauge(object):
    ...
    def avg_ma(self, t_s, pct, cap_mah=700):
        ...
        ★2026-08-12 분모의 `* 10` 제거 — 펌웨어에 있던 버그였다(표시가 정확히
        10배 낮았다). mA = 소모mAh/시간 = (d_pct×용량/100)/(el/3600)
                       = d_pct × 용량 × 36 / el   ← `*10` 이 들어갈 자리가 없다.
        실측 대조: 2.24시간에 73%→58%(-15%p), 700mAh → 47mA 인데 로그는 4mA 였다."""
        if self.t0 is None or t_s <= self.t0:
            return None
        el = t_s - self.t0
        return (self.pct0 - pct) * cap_mah * 36 // el
