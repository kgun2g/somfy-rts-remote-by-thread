...
# ── 펌웨어와 동일한 OCV 곡선 ────────────────────────────────────────────────
# ★2026-08-11 만충 전압 보정 — 상단 3개 앵커만 원곡선 모양을 유지한 채
#   [3980..4200] → [3980..BAT_FULL_MV] 로 압축한다. 중·저 구간은 그대로라
#   무배터리 float 판정(3,970 mV = 78%)은 영향 없다.
#   ★2026-08-13 BAT_FULL_MV = 4,108 (하루 사이 4,158 → 4,128 로 올렸다가 원복).
#   ※이 상수는 "USB 꽂자마자 100% 로 뛰는" 증상의 원인이 **아니다** — 4,044mV(배터리)
#   와 4,152mV(충전 중)는 어느 곡선에서도 90% / 100% 로 같다. 원인은 충전 전류가
#   내부저항을 지나며 단자전압을 108mV 들뜨게 하는 것이다(somfy_app.c 주석 참조).
...
# ★만충 보정으로 곡선 상단이 압축돼 가팔라졌다 → 창을 9 로.
#   (창5 진폭 11%p / 창9 6%p, 추종 지연은 둘 다 1%p 로 동일)
#   ※2026-08-12 2026-08-13 BAT_FULL_MV 는 4108 로 원복됐다(상단 기울기 5.8mV/%p 로 복귀)
#     — 창 9 근거가 다시 유효하다.
WIN = 9
