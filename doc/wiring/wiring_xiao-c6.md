...
#### ★★★2026-08-24 사고 기록 — R5 접촉 불량 (COM7 실기)

**증상**: 잔량 표시가 0% 로 굳거나, 전원 재투입마다 **0% → 10% → 100%** 로 바뀐다.

**측정**:

| | 값 |
|---|---|
| 멀티미터, 배터리 단자 | **4.187V**(USB 분리) / **4.192V**(USB 연결) — 정상 |
| 펌웨어 ADC 환산 | **2,600 mV** (정상값의 62%) |
| 역산한 분압 하단 실효저항 | **45.1 kΩ** (설계 100 kΩ) |

45kΩ 은 **ESP32 내부 풀다운 값과 일치**한다(코드에도 `"내부 풀다운 ~45k"` 진단
항목이 있다 — 전에 CHG_STAT 에서 같은 현상을 겪었다).
⇒ **R5(하단 100k)가 끊긴 채 칩 내부 풀다운만 접지 역할을 하는 상태.**

**소프트웨어 무관 확인**: iot_button 제거/복원 두 빌드에서 2,604 / 2,600mV 로 동일.
마지막 커밋 상태로 되돌려도 동일.

**같은 원인으로 보이는 과거 사건** — 둘 다 NVS 방전기록에 남아 있고, 산포가
1~6카운트로 **깨끗**했다(= ADC 잡음이 아니라 측정 전체가 낮게 나왔다):

```
2026-08-20   4069 → 2651 → 2129 mV   (8시간 걸쳐 3845mV 로 "회복")
2026-08-23   4055 → 3002 mV          (리셋하니 4078mV 로 즉시 정상)
```

접촉이 붙었다 떨어졌다 하면 하단 경로가 R5(100k) ↔ 내부 풀다운(45k)으로 오가며
정확히 이런 값을 만든다.

**조치**: 전원 완전 차단 후 **R5 와 100nF 손납땜 부위 재작업**. 저항 측정으로
100kΩ 확인. 100nF 은 R5 와 **병렬**로 붙이는 자리라 그 작업과 무관하지 않다
(2026-08-19 교체 직후에는 4,010mV·산포 3 으로 정상이었다).

**소프트웨어로 덮지 말 것**: `BOARD_BAT_DIV_BOT` 을 45 로 바꾸면 표시는 맞아지지만
내부 풀다운은 30~60kΩ 로 흔들리고 온도·개체차도 크다. 접촉 불량이면 값이 계속
오락가락한다. 펌웨어에 넣은 **급락 표본 거부**(300mV 초과 낙차를 연속 3회까지 버림,
`sim/tools/bat_spike_reject_sim.py`)는 순간 튐용 안전망일 뿐 **이 결함을 가릴 수 있으므로**
로그(`[BAT%] ★급락 표본 거부`)가 보이면 하드웨어를 의심할 것.

#### ★BAT_ADC 필터 커패시터 (2026-08-12 추가 / 2026-08-19 100nF 로 확정, 필수)
...
| **100nF** | **5 ms** | **31.8 Hz** | **45 ms** | **중앙값 3, 최대 5** | ★**채택(2026-08-19)** |
| 10µF | 500 ms | 0.32 Hz | 4.5초 | 중앙값 3, 최대 4 | 동등(과잉) |
| 100µF | 5 초 | 0.032 Hz | **45초** | 중앙값 3, 최대 4 | 과함(아래 함정) |

- **3카운트는 ESP32 SAR ADC 자체의 잡음 바닥**이다. 100nF·10µF·100µF 이 **셋 다 동일한**
  결과라 100nF 보다 키울 이유가 없다.
...
- ★**2026-08-19 100nF 으로 교체하고 실측 — 위 판정 기준의 "3~5 = 동등(OK)" 에 정확히
  들어왔다.** COM7 실기 20표본(100초, USB 급전, 무선 ON):
  **산포 중앙값 3·평균 2.6·최대 5, 30카운트 이상 0건**, vbat 4010~4014mV(변동 4mV),
  raw 2001~2003(2카운트). → 잡음원이 실제로 kHz 이상이어서 차단 31.8Hz 로 충분했다.
  10µF(τ=500ms)은 **과잉**이었던 셈이고, 100nF 은 100µF 함정(아래)에서 4자릿수 멀다.
  판정 기준은 향후 재검증에도 그대로 쓴다:
  **3~5 = 동등(OK) / 5~10 = 충분 / 15 이상 = 용량 부족(되돌릴 것)**.
- MLCC 는 **DC 바이어스 감쇠**가 있다 — 6.3V급 X5R 10µF 에 2V 가 걸리면 실효 6~7µF.
  결론은 안 바뀌지만 τ 계산 시 감안할 것. **100nF 급(C0G/X7R 소용량)은 감쇠가 거의
  없어** 이 항목에서도 유리하다.
