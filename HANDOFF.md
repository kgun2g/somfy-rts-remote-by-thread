...
> **결론 먼저 (2026-08-25 시점, 두 보드 모두 완료)**
>
> | | C6 | H2 |
> |---|---|---|
> | 잠 | **96.2%** | **96.6%** |
> | 깨어남 | **23.7회/초** | **33.2회/초** |
> | 방전 | **8.7 mV/시간** | — |
> | free heap | 197 KB | **40.5 KB** (11.3 → +29.2) |
> | 방식 | LP 코어 눌림 래치 + ULP 깨움 | **PCF `~INT` 레벨 트리거** |
>
> C6 는 깨어있는 시간 13.4% → **3.6%**, 700mAh 로 약 5일.
> H2 는 **0.0% → 96.6%** — `bt` 락 제거(적용 누락이었다) + `~INT` ISR.
...
### ★★2026-08-25 — H2 는 `~INT` 를 쓸 수 있었다 (C6 결론을 일반화한 것이 오류)

`bt` 락을 푼 뒤 H2 는 깨어남이 **113.9회/초**였다(C6 는 23.7). PM 락 덤프가 이유를
보여줬다 — `I2C_0 NO_LIGHT_SLEEP` 을 **748,352회** 잡았다 놓는다. H2 는 LP 코어가
없어 버튼을 10ms 마다 HW I2C 로 직접 읽기 때문이다. C6 에서 쓴 LP 눌림 래치는
LP 코어가 전제라 H2 엔 못 쓴다.

코드 주석에는 *"`~INT` 는 이 하드웨어에서 쓸 수 없다"* 고 단정돼 있었다. 그런데
그 실측은 **C6 조건**(비트뱅 I2C, GPIO2)이었고 **H2 에서 재본 적이 없었다.**
사용자 지적으로 다시 쟀다:

```
H2  [INTOBS] 표본 16,675  LOW 18(0.1%)  ★전이 36     ← 누르면 선이 움직인다
C6  intdiag  표본 290,023 (버튼 조작 중)  전이 0회    ← 선이 죽어 있다
```

| | 이전(0.0%→) | `bt` 락 제거 | **+ `~INT` ISR** |
|---|---|---|---|
| 잠 | 0.0% | 88.8% | **96.6%** |
| 깨어남 | — | 113.9회/초 | **33.2회/초 (−71%)** |
| 회당 잠 | — | 7.8ms | **29.1ms** |

> **★내가 두 번 틀렸다.** (1) 기존 주석의 "이 하드웨어"를 그대로 믿었다.
> (2) 정지 상태에서 `~INT` 0건인 것을 "조용하다=쓸 수 있다"로 오독했다 —
> **정지 상태의 0건은 아무것도 증명하지 않는다.** 관건은 **누를 때 움직이는가**다.
> 그 오독으로 ISR 을 잘못 걸어 **인터럽트 WDT 재부팅을 44회** 냈다.

**`~INT` 를 다른 보드에서 검토할 때의 절차** (이번에 확립):

1. **ISR 없이 먼저 관찰** — 폴링이 핀 레벨을 같이 보게 해 전이를 센다(`[INTOBS]`).
   ISR 을 먼저 걸면 폭주 시 인터럽트 WDT 로 재부팅한다.
2. **버튼을 누르는 동안** 전이가 늘어나는지 볼 것. 정지 상태 0건은 무의미.
3. 살아 있으면 **레벨 트리거**로 건다. `~INT` 는 레벨 신호라 `NEGEDGE` 는 안 된다 —
   이미 LOW 면 엣지가 안 오고, read 해제 순간의 상승·재하강이 겹치면 ISR 이 몰린다.
   ISR 안에서 **즉시 자기 인터럽트를 끄고**(IRAM-safe `gpio_ll_intr_disable`),
   태스크가 포트를 읽어 **HIGH 로 돌아온 뒤에만** 재활성한다.
   등록은 **초기 read 뒤** — LOW 인 채로 등록하면 레벨 트리거가 즉시 폭주한다.
   (진동 ISR 이 X160 chatter 를 막는 방식과 동일한, 검증된 패턴)
...
| 5 | H2 `bt` 락 제거 | **완료** — `h2_thread`·`c6_thread` 를 `y` 로(gnpe-c6 도 함께 구제). H2 0.0 → 88.8 %, free +29.2 KB |
| 6 | H2 `I2C_0` NO_LIGHT_SLEEP 락 | **완료** — PCF `~INT` 레벨 트리거로 유휴 폴 10 → 50 ms. H2 113.9 → **33.2 회/초, 96.6 %** |
| 7 | C6 메인 루프(10 회/초) + hold_repeat(2 회/초) | **미시도** — 전체가 23.7 회/초라 이제 **비중 절반**이다. 다음 지렛대 |
| 8 | H2 `~INT` 를 C6 에도? | **불가** — C6 는 `intdiag` 실측상 전이 0회(죽은 선). LP 코어 래치가 그 역할을 한다 |

### 현재 남은 병목 (2026-08-25)

두 보드 모두 96 %대에 도달해 **더 줄일 여지가 크지 않다.** 남은 것은:

| 보드 | 깨어남 내역(추정) | 비고 |
|---|---|---|
| C6 23.7 회/초 | 메인 루프 10 + btn 10 + oled 2 + hold 2 | 우리 태스크가 거의 전부 |
| H2 33.2 회/초 | 유휴 폴 20 + 메인 루프 10 + 기타 | `~INT` 안전망 폴이 절반 |

메인 루프(100 ms)를 늘리는 것이 다음 후보지만, **회당 깨어있는 시간이 늘어 상쇄될 수
있다** — C6 에서 btn 폴을 25 → 100 ms 로 늘렸을 때 깨어남은 −28 % 였는데 깨어있는
시간은 오히려 늘었다(1.15 → 1.77 ms). 반드시 **깨어남 횟수가 아니라 깨어있는 시간**
으로 판정할 것.
