...
## ★절전 측정 현황 (2026-08-24 갱신)

> **결론 먼저**: C6 는 깨어있는 시간 **13.4% → 3.6%**, 깨어남 **88.8 → 23.7회/초**,
> 방전 **14.1 → 8.7 mV/시간**. 이 속도면 700mAh 로 **약 5일**.
> H2 는 아직 **light sleep 0.0%** — 2026-08-15 에 고쳤던 `bt` 락이 H2 에만
> 적용 누락돼 그대로 남아 있었다(아래 「2026-08-24」 절).
...
|---|---|---|
| 시작 | 13.4% | |
| `-Os`(그동안 IDF 기본 `-Og` 였다) | 11.9% | 회당 1.52 → 1.35ms |
| 상시 계측 끄기(PM_PROFILING·RUN_TIME_STATS) | 10.7% | 회당 1.20ms |
| 802.15.4 수면 중 전원차단 | (약 5%) | 같은 전압구간 8.37 → 8.94h |
| LP 눌림 래치 + 폴 25→100ms | 11.3% | 깨어남은 줄었으나 회당 시간이 늘어 상쇄 |
| **iot_button 20ms 타이머 제거** | **3.6%** | ★단독으로 나머지 전부보다 컸다 |

### ★★★2026-08-24 — 깨어남의 78% 는 **쓰지도 않는 예제 버튼**이었다

배터리 실측에서 **125만 회 수면 중 20ms 를 넘은 것이 한 번도 없었다.** 버튼 폴을
100ms 로 늘린 뒤에도 그대로였고, 깨어난 원인은 99.97% 가 TIMER(=예약된 알람)인데
우리 태스크 주기는 100/100/500/500ms 뿐이라 12~20ms 를 만들 게 없었다.

→ 콘솔 **`tmr`**(`esp_timer_dump`)로 찍으니 등록된 타이머가 딱 하나였다:

```
Name          Period   Times_trigg   Cb_exec_time
button_timer  20000    10743         60767 us      ← 초당 50회
```

`espressif__button`(iot_button)이 `CONFIG_BUTTON_PERIOD_TIME_MS`(20ms) 주기
esp_timer 를 **상시** 돌리고 있었다. esp-matter light 예제에서 복사돼 온 잔재로,
공장초기화(BOOT 5초 롱프레스) 하나에만 쓰였다. 실제 조작 버튼은 전부 PCF8575 →
`button_handler` 가 처리한다. 사용자 확인 후 **통째로 제거** → `tmr` 출력 타이머 0개.

> **★교훈: 추정하지 말고 찍어라.** 이 하나를 찾기까지 내가 세운 가설이 세 번 틀렸다.
> (1) 배터리 구간 LP 코어 정지 → `LP=ON` 확인으로 반증
> (2) LP 가 PCF 지터로 HP 를 깨움 → `[LPWAKE] HP 깨우기 0.00회/초` 로 반증
> (3) Matter/OpenThread 내부 타이머 → **실제로는 우리 코드**
> `esp_timer_dump()` **한 줄**이 세 번의 추정을 끝냈다.

### ★2026-08-24 — H2 는 왜 아직도 안 자나: 8/15 수정의 **적용 누락**

H2 배터리 13.9시간 실측: `sleep 0.0%`, 그런데 batlog 의 `pm` 열은 **3**
(= light sleep ON + 배터리) — **설정은 정상인데 실제로 안 잔다.**
NVS 로 받은 `esp_pm_dump_locks`:

```
Name   Type            Active  Total_count  Time(%)
bt     APB_FREQ_MAX      1          1        100%    ← 부팅부터 한 번 잡고 안 놓음
rtos0  CPU_FREQ_MAX      1       657888        6%
I2C_0  NO_LIGHT_SLEEP    0        60162        2%
Mode:  APB_MAX 94% / SLEEP 0%
light_sleep_counts:0   reject_counts:0        ← 시도조차 안 함
```

**2026-08-15 에 이미 찾아 고쳤던 그 `bt` 락이다.** 당시 조치
(`CONFIG_USE_BLE_ONLY_FOR_COMMISSIONING=y`)가 **C6 오버레이에만** 들어갔다:

```
sdkconfig.defaults.xiao_c6    ...=y     ← C6 만
sdkconfig.defaults.h2_thread  ...=n     ← H2 는 n 그대로
sdkconfig.defaults.c6_thread  ...=n
```

> **★교훈: 보드별 오버레이 구조에서 "고쳤다"는 한 보드에서만 참일 수 있다.**
> 공통 결함을 고치면 **모든 보드의 defaults 를 확인**할 것.
> (같은 성격의 사고: `g_wake_iter` 정의가 batlog 가드 안에 있어 H2 링크가 깨진 건 — 커밋 a6e5f72)

### 측정 방법 — 반드시 지킬 것

**같은 출발 전압 · 같은 도착 전압으로 비교한다.** OCV 곡선 기울기가 구간마다
**2.3배** 다르기 때문이다(3850~3950 = 5.0mV/%p, 4080~4150 = 11.7mV/%p).
실제로 이걸 어겨 duty 가 다른 두 세션을 연립했다가 **수면 전류 −9.8mA** 라는
불가능한 값이 나온 적이 있다 — 차이는 duty 가 아니라 측정 구간이었다.

도구: `sim/tools/bat_current_estimate.py`(방전→실효 전류 역산),
`sim/tools/batlog_decode.py`(수면 길이 히스토그램·깨어남 원인·태스크별 반복),
`sim/tools/pmdump_decode.py`(PM 락 보유자).

※멀티미터 직렬 전류 측정은 **불가**(이 기판은 배터리 선을 끊을 수 없다).
  배터리 +/− 에 전류계 모드로 직결하면 **셀 단락** — 절대 금지. 전압만 잰다.

---

## 절전 측정 현황 (2026-08-14, 이력)
...
| 순서 | 조치 | 상태 (2026-08-24 갱신) |
|---|---|---|
| 0 | 락 해제 후 방전 측정 | **완료 (C6)** — 96.2% 수면, 8.7mV/h |
| 1 | 라디오 MAC/BB 전원차단 | **완료** — 단 `ESP_PHY_MAC_BB_PD` 가 아니라 **`IEEE802154_SLEEP_ENABLE`** 이다(전자는 Wi-Fi/BLE 용이고 RAM 2~4KB 더 먹는다). 실측 약 5% |
| 2 | CC1101 `SPWD` (송신 안 할 때 절전) | **미시도** (데이터시트 ~1.7mA 추정) |
| 3 | btn_task 유휴 주기 25ms → 100ms | **완료** — 단 주기만 늘리면 "너무 느리다"로 되돌아온다(8/20 실패). LP 눌림 래치 + LP→**태스크** 즉시 깨움이 전제. 결과적으로 반응은 25ms → **3~5ms 로 오히려 빨라짐** |
| 4 | 메인 루프·hold_repeat 깨움 | **미시도** — 합쳐 12회/초. 지금 전체가 23.7회/초라 비중이 커졌다 |
| 5 | **H2 `bt` 락 제거** | **최우선** — `sdkconfig.defaults.h2_thread` 의 `USE_BLE_ONLY_FOR_COMMISSIONING=n` 을 `y` 로. C6 는 이미 적용돼 있다 |
| 6 | H2 `I2C_0` NO_LIGHT_SLEEP 락 | 관찰됨(60,162회/2%). H2 는 LP 코어가 없어 버튼을 10ms 마다 HW I2C 로 읽는다. `bt` 를 푼 뒤 다음 장애물이 될 수 있다 |

### LP 눌림 래치를 쓸 때 반드시 함께 해야 하는 것 (2026-08-23)

주기만 늘리면 **지연이 그대로 체감된다**(8/20 100ms 시도 → "너무 느리다" → 되돌림).
셋이 모두 있어야 한다:

1. **LP 눌림 래치** (`lp_core/pcf_poll.c` `press_latch`) — 폴 사이의 짧은 누름을 OR 로
   모은다. **로터리 A/B 는 제외**(회전 중 토글해 영구 눌림으로 굳는다 — 실제로 겪음),
   **2연속(4ms) 디바운스**(래치는 2ms 글리치도 전부 잡으므로 한 번 거른다).
2. **LP → 태스크 즉시 깨움** — IDF 에 LP→HP 인터럽트 API 가 **없다**
   (`ulp_lp_core_sw_intr_trigger` 는 HP→LP 방향). light sleep **복귀 콜백**에서
   `esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_ULP` 를 보고 통지하는 것이 유일한 통로.
   ★`vTaskDelay` 는 통지로 못 깨운다 — `ulTaskNotifyTake` 로 바꿔야 한다.
3. **`esp_sleep_enable_ulp_wakeup()`** — ★이게 빠지면 2번이 통째로 죽는다.
   실측: 등록 전 ULP 깨움 **0건** → 등록 후 버튼 10회에 **11건**.
